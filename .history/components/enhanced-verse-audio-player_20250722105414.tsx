...
export function EnhancedVerseAudioPlayer({ ayah, totalVerses, className }: EnhancedVerseAudioPlayerProps) {
  ...
  useEffect(() => {
    ...
    const handleEnded = async () => {
      ...
      // Check if auto-play is enabled (connected to main Auto Play setting)
      ...
      if (currentPreferences.autoPlay) {
        try {
          ...
          if (nextVerseNumber <= totalVerses) {
            ...
            console.log(`Auto-playing next verse: ${nextVerseKey} (Auto Play enabled)`)
        ...
        console.log("Auto Play disabled, stopping verse playback")
        handleStop()
      }
    }
