...
export default function SettingsPage() {
  ...
  const [volume, setVolume] = useState<number>(70) // Declare volume state

  // Hydrate preferences on client side and set initial volume
  useEffect(() => {
    try {
      ...
      // Set the volume state based on loaded preferences, converting to percentage
      setVolume(Math.round((loadedPreferences.volume ?? 0.7) * 100))
  ...
  const handlePreferenceChange = (key: keyof UserPreferences, value: any) => {
    ...
    // Special handling for autoPlay, verseAudioEnabled, and volume to update providers directly
    if (key === "autoPlay") {
  ...
  const clearAllData = () => {
    if (
      ...
      setVolume(Math.round((defaultPrefs.volume ?? 0.7) * 100)) // Reset volume slider
      // Also reset provider states
      updateSurahAutoPlay(defaultPrefs.autoPlay)
      updateVerseAudioEnabled(defaultPrefs.verseAudioEnabled)
      ...
      <Card>
        ...
        <CardContent className="space-y-6">
          ...
          <div className="space-y-3">
            ...
            <Slider
              value={[volume]}
              onValueChange={handleVolumeSliderChange} // Use the new handler
              max={100}
              step={1}
              className="w-full"
            />
